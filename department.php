                  ...
                  <tbody>
                    <?php 
                      ...
                      if (isset($_GET['active'])) {
                        $id =$_GET['active'];
                        $setup->dept_active($id); 
                      }
                      if (isset($_GET['inactive'])) {
                        $id =$_GET['inactive'];
                        $setup->dept_inactive($id); 
                      }
                      if (isset($_GET['id'])) {
                        $id =$_GET['id'];
                        $setup->dept_delete($id); 
                      }
                      $obj = $setup->findDept();
                      if($obj->num_rows > 0){
                        while($row = $obj->fetch_assoc()){ 
                          if ($row["status"] == 1) {
                            $status = '<a href="department.php?active='.$row["id"].'" name="active" class="btn btn-success btn-sm"><i class="fa-solid fa-toggle-on"></i></a>';
                          }
                          else{
                            $status = '<a href="department.php?inactive='.$row["id"].'" name="inactive" class="btn btn-secondary btn-sm btn-outline-light"><i class="fa-solid fa-toggle-off"></i></a>';
                          }
                         ?>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            ...
                            <a href="department.php?id=<?php echo $row["id"]; ?>" class="btn btn-danger">Confirm</a>
                        </div>
                    </div>
                </div>
            </div>

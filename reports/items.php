...
class PDF extends PDF_MC_Table
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 5, 'List of Assets', 0, 1, 'C');
        // Line break
        $this->Ln(6);
...
$this->Cell(66, 10, 'Specification', 1, 0, 'C');
// $this->Cell(22, 10, 'Status', 1, 0, 'C');
$this->Cell(47, 10, 'Remarks', 1, 1, 'C');
...
// $result = $pdf->con->query("SELECT * FROM tbl_products where $get ORDER BY `dept` ASC, `user` ASC;");
$result = $pdf->con->query("SELECT * 
FROM tbl_products 
WHERE $get 
  AND status != 4 
ORDER BY dept ASC, user ASC;
");
...
$pdf->SetFont('Arial', '', 8.2);
    ...
    // Add row to the PDF
    $pdf->SetWidths([12, 20, 42, 26, 35, 21, 28, 50, 66, 47]);
